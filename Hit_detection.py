...
def tennis_hit_detection(model_path, video_source=0, line_start=(0, 270), line_end=(640, 300), 
                         save_folder="tennis_hit_records", conf_threshold=0.5, cooldown=1.0):
    """
    Detects tennis ball hits crossing a specified line and saves frames when hits occur.
    ...
    Usage example:
    tennis_hit_detection("/path/to/model.pt", video_source=3, 
                         line_start=(0, 270), line_end=(640, 300))
    """
    ...
    while True:
        ...
        for box in results.boxes:
            ...
            if cls == 0:  # Tennis ball class
                ...
                if check_line_box_intersection(line_start, line_end, (x1, y1, x2, y2)):
                    cv2.putText(frame, "Hit Detected!", (50, 50), 
        ...
        if hit_detected and (time.time() - last_save_time) > cooldown:
        ...
        cv2.imshow("Tennis Hit Detection", frame)
